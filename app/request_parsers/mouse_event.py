...
def _parse_button_state(buttons):
    ...
    if not (0 <= buttons <= _MAX_BUTTON_STATE):
        raise InvalidButtonStateError(f'Button state must be <= 0x{_MAX_BUTTON_STATE}x: {buttons}')
    return buttons
...
def _parse_relative_position(relative_position):
    ...
    if not (0.0 <= relative_position <= 1.0):
        raise InvalidRelativePositionError(f'Relative position must be a float between 0.0 and 1.0: {relative_position}')
