...
def python_project_license_get(project):
    ...
    try:
        license_path = _get_license_pattern(project)
    except KeyError:
        return flask.make_response('Unknown project', 404)

    if _is_url(license_path):
        return _make_redirect_response(license_path)
    return _make_plaintext_response(license_path)


def _get_license_pattern(project):
    return _LICENSE_FILE_GLOB_PATTERNS[project]


def _is_url(license_pattern):
    return license_pattern.startswith('http')
